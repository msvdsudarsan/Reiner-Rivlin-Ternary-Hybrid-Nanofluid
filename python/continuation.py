...
pseudo-arclength scheme, an upgrade over simple natural-parameter-only
continuation, which is unable to cross a genuine fold on its own.
"""
...
def trace_branch(K, S, lam_start=0.0, lam_stop_natural=-2.5, ds0=0.05, max_palc_steps=250, eta_inf=15.0):
    ...
    then switch to a reduced-plane pseudo-arclength (PALC) diagnostic and
    continue for up to max_palc_steps additional points, testing whether
    continuation beyond the natural-continuation limit is possible. In
    the runs reported in the manuscript, this diagnostic reaches the same
    termination point as natural continuation but does not cross it,
    consistent with a regularity boundary rather than a fold."""
    ...
    # ---- stage 2: reduced-plane PALC diagnostic near the regularity boundary ----
    if len(points) >= 2:
