...
the corrected species perturbation equation analytically shows why:
the two characteristic roots of that limiting constant-coefficient
equation have product `gamma - Sc_thnf*beta`, independent of the
base-state value at infinity. Both roots are negative (decaying) for
any `gamma > Sc_thnf*beta` (given H(infinity) < 0 at the baseline),
which is a much more restrictive condition than the physically
relevant low-gamma range explored here, yet the far-field decay
condition is nonetheless satisfied smoothly with no crossing anywhere
in the range gamma in [-50, 400] actually tested by shooting -- an
honest finding that complicates, rather than confirms, the simplest
local picture. The corrected analytical threshold is `gamma_thr =
+Sc_thnf*beta` (NOTE: an earlier revision of this repository and the
manuscript both had this threshold sign-flipped, `gamma_thr =
-Sc_thnf*beta`; see symbolic_derivation/derive_species_sign_check.py
for the independent re-derivation that caught and fixed this). See
