...
def _rhs(eta, z, gamma, K, base_interp):
    ...
    # CORRECTED (V26): an independent symbolic re-derivation of manuscript
    # Eq. (linG) -- all eight K-bracket terms, then eliminating
    # H0''=-2F0', Hf''=-2Ff' exactly as stated beneath Eqs. (linF)-(linPhi)
    # -- found this formula was missing the term 2*K*(Ff*G0pp - Gf*F0pp)
    # relative to the manuscript's own printed equation. Confirmed by
    # symbolic subtraction (result identically zero only once the term is
    # restored). See symbolic_derivation/derive_stability_gfpp_check.py.
    Gfpp = (4*K*Ff*G0pp - 2*K*Gf*F0pp - 2*K*F0p*Gfp - 2*K*Ffp*G0p - gamma*Gf
